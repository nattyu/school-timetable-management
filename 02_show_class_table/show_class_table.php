               ...
               <?php 
                  for ($i = 0; $i < count($result_subject) ; $i++){
                     if ($defalt_class_num == $i) {
                        echo '<option value="' . $result_subject[$i][0] . '" selected>' . $result_subject[$i][1] . '</option>';
                     } else {
                        echo '<option value="' . $result_subject[$i][0] . '">' . $result_subject[$i][1] . '</option>';
                     }
                  }
               ?>
            </select>
         ...
         <div class="container-grid">
            ...
            <div class="grid-area">
               <?php for ($i = 1; $i <= 6; $i++) {
                  for ($j = 0; $j < 5; $j++) {
                     echo '<div class="item">';
                        echo '<p class="subject">' . $result_subject[$defalt_class_num][$i + 1 + 6 * $j] . '</p>';
                        echo '<p class="teacher">' . $result_teacher[$defalt_class_num][$i + 1 + 6 * $j] . '</p>';
                        echo '<p class="place">' . $result_place[$defalt_class_num][$i + 1 + 6 * $j] . '</p>';
                     echo '</div>';
                  }
               } ?>
            </div>
